...
from networked_game.game_engine.game_engine import Command
from networked_game.game_engine.piece_util_functions import (
    get_piece_position,
    get_player_from_username,
    move_piece,
)
...
class Move(Command):
    def __init__(self):
        ...
        self.rules.append(MustOwnPiece())
    ...
    def process(self, data, user_connection, game_data) -> dict:
        ...
        if "actions" in board["piece_positions"][destination_position]:
            for action_name in board["piece_positions"][destination_position][
                "actions"
            ]:
                if action_name == "get_resources":
                    get_resources = board["piece_positions"][destination_position][
                        "actions"
                    ]["get_resources"]
                    for resource_name in get_resources:
                        resource_value = get_resources[resource_name]
                        board["players"][player_name]["resources"][
                            resource_name
                        ] += resource_value
                if action_name == "pick_quest_card":
                    board["round_moves"].insert(
                        0, {"action": "pick_quest_card", "player": player_name}
                    )
